...
public class ListaAlocSecGames {
    ...
    //lista 
    public static class ListaGames {
        private Game[] array;
        private int n;

        public ListaGames() { this(1000); }
    ...
    public static List<Game> readFromCSV(String filePath) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                Game g = new Game();
                g.setId(fields[0]); g.setName(fields[1]); g.setReleaseDate(fields[2]);
                g.setEstimatedOwners(fields[3]); g.setPrice(fields[4]); g.setSupportedLanguages(fields[5]);
                g.setMetacriticScore(fields[6]); g.setUserScore(fields[7]); g.setAchievements(fields[8]);
                g.setPublishers(fields[9]); g.setDevelopers(fields[10]); g.setCategories(fields[11]);
                g.setGenres(fields[12]); g.setTags(fields[13]);
                games.add(g);
            }
        } catch (IOException e) {
    ...
    public static void main(String[] args) {
        ...
        // csv
        List<Game> todosGames = readFromCSV("/tmp/games.csv");
        ListaGames lista = new ListaGames(); 

        //Pre-carregamento
        while (true) {
            String input = "";
            if (sc.hasNextLine()) input = sc.nextLine().trim();
            
            if (Parada(input)) break;

            try {
                int id = Integer.parseInt(input);
                Game g = findById(todosGames, id);
                if (g != null) {
                    lista.inserirFim(g);
                }
            } catch (Exception e) {
                //ignorar entradas inválidas ou erros de inserção
            }
        }
        ...
        if (sc.hasNextLine()) {
            try {
                int nComandos = Integer.parseInt(sc.nextLine().trim());

                for (int i = 0; i < nComandos; i++) {
                    if (!sc.hasNextLine()) break;

                    String linhaComando = sc.nextLine().trim();
                    ...
                    try {
                        switch (comando) {
                            case "II":
                                ...
                                Game gII = findById(todosGames, idII);
                                if (gII != null) lista.inserirInicio(gII);
                                break;
                            case "IF":
                                int idIF = Integer.parseInt(partes[1]);
                                Game gIF = findById(todosGames, idIF);
                                if (gIF != null) lista.inserirFim(gIF);
                                break;
                            case "I*":
                                int posI = Integer.parseInt(partes[1]);
                                int idI = Integer.parseInt(partes[2]);
                                Game gI = findById(todosGames, idI);
                                if (gI != null) lista.inserir(gI, posI);
                                break;
                            case "RI":
                                gameRemovido = lista.removerInicio();
                                break;
                            case "RF":
                                gameRemovido = lista.removerFim();
                                break;
                            case "R*":
                                int posR = Integer.parseInt(partes[1]);
                                gameRemovido = lista.remover(posR);
                                break;
                        }
                        ...
                        // ignora erros de posição inválida
                    }
                }
            } catch (Exception e) {
            }
